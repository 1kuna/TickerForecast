...
tickers = ['BRK-B', 'GOOG', 'META', 'V', 'WMT',
           'TSM', 'JPM', 'JNJ', 'PG', 'DIS', 'IBM',
           'BAC', 'AMD', 'NFLX', 'SBUX']

## DO LATER
## 'BA', 'CAT', 'MRK', 'INTC', 'ORCL'

# Create sets to store the file suffixes
sets = ['TRAIN', 'VAL', 'TEST']
...
for ticker in tickers:
    ...
    if not os.path.exists(get_file_path(f'intraday\\TICKERS\\NEW\\{ticker}')):
        os.makedirs(get_file_path(f'intraday\\TICKERS\\NEW\\{ticker}'))
    start_directory = get_file_path(f'intraday\\TICKERS\\NEW\\{ticker}')
    ...
    for x in range(1,3):
        for i in range(1,13):
            ...
            with requests.Session() as s:
                ...
                with open(get_file_path(start_directory, filename=f'{name}.csv'), mode='w', newline='') as csv_file:
                    writer = csv.writer(csv_file)
                    writer.writerows(my_list)
                    print(f'Year: {x} Month: {i} downloaded ({name}.csv)')

    df = pd.DataFrame()
    df = pd.concat([pd.read_csv(f, index_col='time') for f in glob.glob(get_file_path(start_directory, filename='*.csv'))])

    # Drop all rows with NaN values and sort by date and time
    df.sort_index(inplace=True)
    df.drop_duplicates(inplace=True)

    df.to_csv(get_file_path(start_directory, filename=f'{ticker}_COMBINED.csv'))
    df = pd.read_csv(get_file_path(start_directory, filename=f'{ticker}_COMBINED.csv'))
    ...
    if not os.path.exists(get_file_path(f'intraday\\TICKERS\\NEW\\{ticker}\\training')):
        os.makedirs(get_file_path(f'intraday\\TICKERS\\NEW\\{ticker}\\training'))
    result_directory = get_file_path(f'intraday\\TICKERS\\NEW\\{ticker}\\training')

    # Save the data to a csv file
    train.to_csv(get_file_path(result_directory, filename=f'{ticker}_TRAIN.csv'))
    val.to_csv(get_file_path(result_directory, filename=f'{ticker}_VAL.csv'))
    test.to_csv(get_file_path(result_directory, filename=f'{ticker}_TEST.csv'))

    # Save the data to a parquet file
    train.to_parquet(get_file_path(result_directory, filename=f'{ticker}_TRAIN.parquet'))
    val.to_parquet(get_file_path(result_directory, filename=f'{ticker}_VAL.parquet'))
    test.to_parquet(get_file_path(result_directory, filename=f'{ticker}_TEST.parquet'))
...
for set in sets:
    for ticker in tickers:
        ...
        dir_path = get_file_path(f'intraday\\TICKERS\\NEW\\{ticker}\\training')
        ...
        matrix.to_csv(get_file_path(f'intraday\\TICKERS\\NEW', filename=f'{set}_COMBINED.csv'), index=False)
        matrix.to_parquet(get_file_path(f'intraday\\TICKERS\\NEW', filename=f'{set}_COMBINED.parquet'), index=False)
    dfs = []